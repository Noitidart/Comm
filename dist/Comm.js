...
var Comm = {
	...
	server: {
		...
		framescript: function framescript(aChannelId) {
			/* global Services.mm */
			...
			this.listener = {
				receiveMessage: function (e) {
					...
					if (payload.method) {
						...
						var rez_scope = scope[payload.method](payload.arg, payload.cbid ? this.reportProgress.bind({ THIS: this, cbid: payload.cbid, messageManager: messageManager }) : undefined, this, messageManager, browser); // only on bootstrap side, they get extra 2 args
						// in the return/resolve value of this method call in scope, (the rez_blah_call_for_blah = ) MUST NEVER return/resolve an object with __PROGRESS:1 in it
						if (payload.cbid) {
							if (rez_scope && rez_scope.constructor.name == 'Promise') {
								rez_scope.then(function (aVal) {
		...
		content: function content(aContentWindow, onHandshakeComplete, aPort1, aPort2, aPortGenerationWithWorker) {
			var type = 'content';
			var category = 'server';
			var scope = gCommScope;
			Comm[category].instances[type].push(this);
			...
			if (!aPort1) {
				if (aPortGenerationWithWorker) {
					...
					var onhidwinload = function onhidwinload() {
