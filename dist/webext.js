...
if (typeof gCommScope == 'undefined') {
	// optional global, devuser can specify something else, and in case of Comm.client.framescript he will have to
	var gCommScope = undefined;
}

var Comm = {
	unregister_generic: function unregister_generic(category, type, self) {
	...
	server: {
		webextports: function webextports() {
			/*
   ...
   */
			...
			this.listener = function handleMessage(payload) {
				...
				if (payload.method) {
					if (!(payload.method in scope)) {
						console.error('method of "' + payload.method + '" not in scope');throw new Error('method of "' + payload.method + '" not in scope');
					} // dev line remove on prod
					...
					if (payload.cbid) {
						if (rez_scope && rez_scope.constructor.name == 'Promise') {
							rez_scope.then(function (aVal) {
								// console.log('Comm.'+category+'.'+type+' - Fullfilled - rez_scope - ', aVal);
								this[messager_method](portname, payload.cbid, aVal);
							}.bind(this), genericReject.bind(null, 'rez_scope', 0)).catch(genericCatch.bind(null, 'rez_scope', 0));
						} else {
					...
					console.error('Comm.' + category + '.' + type + ' - invalid combination. method:', payload.method, 'cbid:', payload.cbid, 'payload:', payload);throw new Error('Comm.' + category + '.' + type + ' - invalid combination');
				}
			}.bind(this);
		...
		webextexe: function webextexe(aAppName, onConnect, onFailConnect) {
			/*
   used as setup from background.js
   so in background.js do
   ...
   */
			...
			this.listener = function handleMessage(payload) {
				...
				if (!connected) {
					if (payload == 'CONNECT_CONFIRMATION') {
						// crossfile-link994844
						connected = true;
						...
						console.error('has not proven connection yet by sending string of "connected"!');
					}
					return;
				}
		...
		webext: function webext(aWebextEngine) {
			/*
   ...
   */
			...
			this.listener = function handleMessage(payload) {
				...
				if (payload.method) {
					if (!(payload.method in scope)) {
						console.error('Comm.' + category + '.' + type + ' - method of "' + payload.method + '" not in scope');throw new Error('method of "' + payload.method + '" not in scope');
					} // dev line remove on prod
					var rez_scope = scope[payload.method](payload.arg, payload.cbid ? this.reportProgress.bind({ THIS: this, cbid: payload.cbid }) : undefined, this);
					// in the return/resolve value of this method call in scope, (the rez_blah_call_for_blah = ) MUST NEVER return/resolve an object with __PROGRESS:1 in it
					if (payload.cbid) {
			...
			var onConnect = function (aPort) {
				if (aPort.name == 'bootstrap-comm') {
					...
					console.warn('name of aPort coming to Comm.server.webext IS NOT bootstrap-comm it is:', aPort.name);
				}
			}.bind(this);

			var port;
			var browser;
			aWebextEngine.startup().then(function (api) {
				browser = api.browser;
		...
		worker: function worker(aWorkerPath, onBeforeInit, onAfterInit, onBeforeTerminate, aWebWorker) {
			...
			this.unregister = function () {
				...
				var unregIt = function unregIt(aCaught) {
					if (aCaught) {
				...
				if (worker && onBeforeTerminate) {
					...
					if (rez_preterm && rez_preterm.constructor.name == 'Promise') {
						...
						rez_preterm.then(unregIt, unregIt).catch(function (aCaught) {
							return unregIt.bind(null, aCaught);
						});
					} else {
						unregIt();
					}
			...
			this.createWorker = function (onAfterCreate) {
				...
				if (onAfterInit) {
					...
					onAfterInit = function onAfterInit(aArg, aComm) {
		...
		content: function content(aContentWindow, onHandshakeComplete) {
			...
			this[messager_method] = function (aMethod, aArg, aCallback) {
				...
				if (aArg && aArg.__XFER) {
					...
					if (Array.isArray(__XFER)) {
						...
						try {
							for (var _iterator2 = (0, _getIterator3.default)(__XFER), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
								var p = _step2.value;
		...
		unregAll: function unregAll(aType) {
			var category = 'server';
			...
			try {
				for (var _iterator3 = (0, _getIterator3.default)(type_instances_clone), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
	...
	client: {
		webextports: function webextports(aPortType) {
   ...
   */
			...
			this[messager_method] = function (aMethod, aArg, aCallback) {
				...
				if (aArg && aArg.__XFER) {
					...
					if (Array.isArray(__XFER)) {
						...
						try {
							for (var _iterator4 = (0, _getIterator3.default)(__XFER), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
								var p = _step4.value;
		...
		content: function content(onHandshakeComplete) {
			var type = 'content';
			var category = 'client';
			var scope = gCommScope;
			Comm[category].instances[type].push(this);
			...
			this[messager_method] = function (aMethod, aArg, aCallback) {
				...
				if (aArg) {
					if (aArg.__XFER) {
						...
						xferScope = aArg;
					} else if (aArg.a && aArg.m && aArg.a.__XFER) {
						// special handle for callIn***
				...
				if (xferScope) {
					...
					if (Array.isArray(xferIterable)) {
						...
						try {
							for (var _iterator5 = (0, _getIterator3.default)(xferIterable), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
								var p = _step5.value;
			...
			this.listener = function (e) {
				...
				if (payload.method) {
					if (!(payload.method in scope)) {
						console.error('Comm.' + category + '.' + type + ' - method of "' + payload.method + '" not in WINDOW');throw new Error('method of "' + payload.method + '" not in WINDOW');
					} // dev line remove on prod
					var rez_scope = scope[payload.method](payload.arg, payload.cbid ? this.reportProgress.bind({ THIS: this, cbid: payload.cbid }) : undefined, this);
					// in the return/resolve value of this method call in scope, (the rez_blah_call_for_blah = ) MUST NEVER return/resolve an object with __PROGRESS:1 in it
					// console.log('Comm.'+category+'.'+type+' - rez_scope:', rez_scope);
			...
			var winMsgListener = function (e) {
				...
				switch (data.topic) {
					case 'contentComm_handshake':
						...
						break;default:
						console.error('Comm.' + category + '.' + type + ' - unknown topic, data:', data);
				}
			}.bind(this);
		...
		unregAll: function unregAll(aType) {
			...
			try {
				for (var _iterator6 = (0, _getIterator3.default)(type_instances_clone), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
					var inst = _step6.value;
	...
	callInX2: function callInX2(aCommTo, aCallInMethod, aMessageManagerOrTabId, aMethod, aArg, aCallback) {
	...
	callInX: function callInX(aCommTo, aCallInMethod, aMethod, aArg, aCallback, aMessageManager) {
		// MUST not be used directly, MUSt have aCommTo and aCallInMethod bounded
		aCommTo = typeof aCommTo == 'string' ? gCommScope[aCommTo] : aCommTo;
		var messagerMethod;
		if (aCommTo.copyMessage) {
...
var CommHelper = {
	...
	webextexe: {
		callInBackground: function callInBackground() {},
		callInThread: function callInThread() {}
	},
	webextexethread: {
		callInExe: function callInExe() {},
		callInBackground: function callInBackground() {}
	}
};
